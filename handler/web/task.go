...
func (t *taskWeb) TaskAddProcess(c *gin.Context) {
	...
	if status == 201 {
		...
		c.Redirect(http.StatusSeeOther, "/client/modal?status=error&message=Add Task Succes!")
	}
}
...
func (t *taskWeb) TaskDeleteProcess(c *gin.Context) {
	...
	if status == 201 {
		...
		c.Redirect(http.StatusSeeOther, "/client/modal?status=error&message=Delete Task Success!")
	}
}
